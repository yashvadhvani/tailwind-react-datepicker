  ...
  >
    <div className="grid grid-cols-7 gap-y-0.5 my-1">
      {calendar.date().map((date: any, keyDate: number) => (
        ...
        >
          {(isBetweenRange(date) || date.hovered()) && (
            <CSSTransition
              timeout={5000}
              classNames={{
                enter: 'opacity-0',
            ...
            >
              <span
                className={`absolute bg-litepie-primary-100 bg-opacity-60 dark:bg-litepie-secondary-700 dark:bg-opacity-50 ${betweenRangeClasses(
                  date,
                )}`}
              />
            </CSSTransition>
          ...
          <button
            type="button"
            className={`litepie-datepicker-date relative w-12 h-12 lg:w-10 lg:h-10 flex justify-center items-center text-xs 2xl:text-sm focus:outline-none ${datepickerClasses(
              date,
